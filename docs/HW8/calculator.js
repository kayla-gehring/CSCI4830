...
//tracks if new event or not
var newEvent = true;
var keyUpCount = 0;
var keyDownCount = 0;
var eventTime = 0;

//holding they key scrolls
//double clicking a key indicates selection of column/row/key
//triple clicking a key indicates go back a step (if selecting row -> go to select column)
$(document).on('keyup keydown', function(event) {
    setInterval(keyPressEvents(event), 250); //check for event every 250 ms

    //tracks whether selecting column or row
    var rowScrolling = false;
    var colScrolling = false;

    function keyPressEvents(event) {
        if (newEvent){ //else, skip function
            //detect keypress, keyhold, double press
            var time = 0;
            if (event.type == "keydown") {
                //new key selection event
                if (rowScrolling == false && colScrolling == false){
